...
from .discriminator_model import PG_MultiScaleDiscriminator, PG_MultiPatchDiscriminator
from .networks import GANLoss, get_scheduler, update_learning_rate
import torch
...
#input_nc, output_nc, norm='batch', use_dropout=False, init_type='normal', gpu_ids=[]
        self.net_g = networks.define_G(opt.input_nc, opt.output_nc,
                                      opt.norm, not opt.no_dropout, opt.init_type, self.gpu_ids).cuda()
        self.net_d = networks.define_D(opt.input_nc + opt.output_nc, opt.ndf,
                                          opt.n_layers_D, opt.norm, opt.init_type, self.gpu_ids).cuda()
        self.net_d_tf = networks.define_D(opt.input_nc + opt.output_nc, opt.ndf,
                                          opt.n_layers_D, opt.norm, opt.init_type, self.gpu_ids).cuda()
